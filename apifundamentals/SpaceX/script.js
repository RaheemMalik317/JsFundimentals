...
/*
    ...
    -Allows us access points(s) to the server.
 ...
 *      -Is a part of a browser window, NOT JavaScript.
...
Promise(s)
    Always one of three states
        -Pending
            -Inital state, neither fulfilled or rejected.
            -Fulfilled
...
//        4
     ...
     *      1: fetch() method starts the process of fetching a resource from a network. Returns a promis which is fulfilled once the response is available. 
            2: Capture the results from the API in a promise resolver, received as a JSON*
...
// When working with any API, we will always want to console.log the response to see how the data is structured so we can effectively step through it and grab what we need. (You will alays need something in the () for it to show correctly on the website!)
...
searchForm.addEventListener('submit',fetchSpace); //javascript or JS is always cap-sensitve
